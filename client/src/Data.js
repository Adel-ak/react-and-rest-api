...
export default class Data {
   ...
   * there are 3 kinds of http status can respond from the request
   * 401 will give back - 'Unauthorized'
   * 200 will give back - 'OK'
   * 500 will give back - 'Server error'
  ...
  async getUser(email, password) {
    ...
    if (response.status === 200) {
      ...
      //if status is 401 an error noting to the user if email or password is wrong;
      return {
        isNull: true,
        errors: [res.errors[1]]
      };
   ...
   * there are 3 kinds of http status can respond from the request
   * 400 will give back - 'SequelizeValidationError' (if any)
   * 201 will give back - 'Success/Created'
   * 500 will give back - 'Server error'
   * errors will always throw an object
   ...
   * there are 2 kinds of http status can respond from the request
   * 404 will give back - ''Course not found'
   * 200 will give back - 'OK'
   * 500 will give back - 'Server error'
   * errors will always throw an object
   * @param {string} path value of api endpoint
   ...
   * there are 3 kinds of http status can respond from the request
   * 404 will give back - 'SequelizeValidationError'
   * 201 will give back - 'Success/Created'
   * 500 will give back - 'Server error'
   * errors will always throw an object
   * @param {string} path value of api endpoint
 ...
 * 403 will give back - 'Unable to update other users\'s courses'
 * 404 will give back - 'Courses not found / Unable to update'
 * 204 will give back - 'Success/No content'
 * 500 will give back - 'Server error'
 * errors will always throw an object
